...
bool redsel = true;
bool blusel = false;
int vmin = 5, vmax = 256, smin = 20;
int redhmin = 170, redhmax = 180, bluhmin = 80, bluhmax = 95;
int ampthresh = 20;
int kernel_size = 41;
...
int main(int argc, const char** argv)
{
	...
	ofstream redARfile("redARfile.bin", ios::binary);
	ofstream bluARfile("bluARfile.bin", ios::binary);
	ofstream redANfile("redANfile.bin", ios::binary);
	ofstream bluANfile("bluANfile.bin", ios::binary);
	
	namedWindow("Color Tracker", 0);
	setMouseCallback("Color Tracker", onMouse, 0);
	createTrackbar("Vmin", "Color Tracker", &vmin, 256, 0);
	createTrackbar("Vmax", "Color Tracker", &vmax, 256, 0);
	createTrackbar("RedHmin", "Color Tracker", &redhmin, 180, 0);
	createTrackbar("RedHmax", "Color Tracker", &redhmax, 180, 0);
	createTrackbar("BluHmin", "Color Tracker", &bluhmin, 180, 0);
	createTrackbar("BluHmax", "Color Tracker", &bluhmax, 180, 0);
	createTrackbar("Smin", "Color Tracker", &smin, 256, 0);
	createTrackbar("Amplitude Threshold", "Color Tracker", &ampthresh, 256, 0);
	createTrackbar("Speed", "Color Tracker", &dt, 200, 0);
	...
	if (argc > 1){
		...
		char* def_video = "C:\\Users\\Radu Darie\\Google Drive\\ODBS\\practice_video\\20141119_Leon_Tunnel_1.mpg";
	...
	for (;;)
	{
		...
		if (!paused)
		{
			...
			if (trackRed){
				...
				// Set red region of interest
				rectangle(image, Rect(0, 0, blackout_width, blu_backproj.rows), Scalar(0, 0, 0), CV_FILLED);
				rectangle(red_backproj, Rect(0, 0, blackout_width, blu_backproj.rows), Scalar(0, 0, 0), CV_FILLED);
				...
				if (red_trackBox.size.area() >= 1){
					...
					Size red_size = red_trackBox.size;
					int red_area = red_size.area();
					redARfile.write((char *)&red_area, sizeof(red_area));
					float red_angle = red_trackBox.angle;
					redANfile.write((char *)&red_angle, sizeof(red_angle));
				}
				else{
					int dummycoord = 5000;
					float dummyangle = 500;
					redXYfile.write((char *)&dummycoord, sizeof(dummycoord));
					redXYfile.write((char *)&dummycoord, sizeof(dummycoord));
					redANfile.write((char *)&dummyangle, sizeof(dummyangle));
					redARfile.write((char *)&dummycoord, sizeof(dummycoord));
				}
			} // end if track red
			else{
				int dummycoord = 10000;
				float dummyangle = 1000;
				redXYfile.write((char *)&dummycoord, sizeof(dummycoord));
				redXYfile.write((char *)&dummycoord, sizeof(dummycoord));
				redANfile.write((char *)&dummyangle, sizeof(dummyangle));
				redARfile.write((char *)&dummycoord, sizeof(dummycoord));
			...
			if (trackBlu){
				...
				// Set blue region of interest
				rectangle(image, Rect(0, 0, blackout_width, blu_backproj.rows), Scalar(0, 0, 0), CV_FILLED);
				rectangle(blu_backproj, Rect(0, 0, blackout_width, blu_backproj.rows), Scalar(0, 0, 0), CV_FILLED);
				...
				if (blu_trackBox.size.area() >= 1){
					...
					Size blu_size = blu_trackBox.size;
					int blu_area = blu_size.area();
					bluARfile.write((char *)&blu_area, sizeof(blu_area));
					float blu_angle = blu_trackBox.angle;
					bluANfile.write((char *)&blu_angle, sizeof(blu_angle));
				}
				else{
					int dummycoord = 5000;
					float dummyangle = 500;
					...
					bluXYfile.write((char *)&dummycoord, sizeof(dummycoord));
					bluANfile.write((char *)&dummyangle, sizeof(dummyangle));
					bluARfile.write((char *)&dummycoord, sizeof(dummycoord));
				}
			} // end if track blue
			else{
				int dummycoord = 10000;
			...
			if (red_timeout == 0 && red_trackBox.size.area() >= 1){
				...
				(image, red_trackBox.center, 1, Scalar(255, 0, 255), 5, 1, 0);
			}
			else if (trackRed){
